...
$("button").on("click", function(){
    ...
    //validate email
    if(!emailValidation){
        $("input#email").css("border", "1px solid rgb(249 0 0)").addClass("onError");
        $(".errorMessage.email").css({"display": "block", "text-align": "left"});
        
    }
    else{
        $("input#email").css({"border": "1px solid hsl(249, 10%, 26%)"}).removeClass("onError");
        $(".errorMessage.email").css("display", "none");
    }

    //validate passwd
    if(!passwdValidation){
        $("input#passwd").css("border", "1px solid rgb(249 0 0)").addClass("onError");
        $(".errorMessage.pass").css({"display": "block", "text-align": "left"});
    }
    else{
        $("input#passwd").css({"border": "1px solid hsl(249, 10%, 26%)"}).removeClass("onError");
